...
class Waymo(data.Dataset):
    def __init__(self, root_dir, split, cfg):
        ...
        self.num_classes = 3
        self.max_objs = 50
        self.class_name = ['Pedestrian', 'Car', 'Cyclist', 'Sign']
        self.cls2id = {'Pedestrian': 0, 'Car': 1, 'Cyclist': 2, 'Sign': 3}
        self.resolution = np.array([768, 512]) if 'resolution' not in cfg.keys() else np.array(cfg['resolution']) # W * H
    ...
    def __getitem__(self, item):
        ...
        if self.split!='test':
            ...
            for i in range(object_num):
                ...
                # filter objects by num_lidar_points_ber_box.
                # num_lidar_points_ber_box are a proxy for occlusion.
                ...
                num_lidar_points_ber_box = objects[i].score
                if objects[i].cls_type == "Car" and num_lidar_points_ber_box <= 100:
                    continue
                if objects[i].cls_type != "Car" and num_lidar_points_ber_box <= 50:
                    continue
